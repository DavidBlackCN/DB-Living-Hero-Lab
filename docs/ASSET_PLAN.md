...
| Normal v2 | `public/assets/hero/normal/base-normal-v2.png` | Integrated semantic refinement candidate / **not frozen** | 与冻结 Base 严格同为 1672×941 RGB、逐像素注册；脸、发、衣物、石材和天空采用少量离线语义区域修正，已接入 Normal View / Test Light。仍需校正细发束、人物与背景遮挡和建筑层次后再考虑 Runtime Lighting 冻结。详见 [`validation/NORMAL_V2_VALIDATION.md`](validation/NORMAL_V2_VALIDATION.md)。 |
...
Debug 面板不再提供整图闭眼候选切换。运行时只加载局部双眼 PNG；坐标以 Base 源图左上角为原点，左眼 `[1080,177,1172,250)`、右眼 `[1152,195,1244,273)`。当前 Normal v2 资源 URL 在 `src/config/hero.ts`，可独立替换并按 Base 尺寸重新验收。v2 的语义区域只存在于离线生成脚本，没有新增运行时 Mask 图集。
