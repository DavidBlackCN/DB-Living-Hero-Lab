...
| Blink Closed | `public/assets/hero/blink/blink-closed-eyes-v1.png` | Full-frame rejected / local-eye source candidate | v1 与本轮重新生成的 v2 均存在眼外全域重绘，整图切换不可用；v2 失败原图仅归档于 `docs/validation/`。后续转局部眼部方案，详见 [`validation/BLINK_V2_FULL_FRAME_RETRY.md`](validation/BLINK_V2_FULL_FRAME_RETRY.md)。 |
...
Debug 面板可静态切换 Base 与 v1 闭眼候选图，便于观察注册和背景变化。该开关仅用于查看被拒绝的整图方案，不代表运行时眨眼实现。v2 失败候选没有接入 asset config。
