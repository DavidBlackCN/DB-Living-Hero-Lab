...
Normal v3 由 `scripts/generate_normal_v3.py` 以 v2 为像素基底局部合成：窄发束沿各自路径赋予方向，交叠背景缝隙退到近中性；袖子、背心和裙摆内部再低通，远景塔楼／墙面与下方石雕降低方向幅度。脸、天空、主石柱和栏杆在输出前直接恢复 v2 原像素。`scripts/validate_blink_normal.py` 验证尺寸、局部改动边界与受保护区域；`scripts/diagnose_normal_v3.py` 生成四组局部六光向／1.3× stress 对照。页面 Normal URL 已指向 v3。当前 Normal 阶段的冻结依据见 [`validation/NORMAL_V3_VALIDATION.md`](validation/NORMAL_V3_VALIDATION.md)；没有实现正式 24h Runtime Lighting。
