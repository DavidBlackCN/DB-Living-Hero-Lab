...
R2A 在独立的 `src/config/lighting.ts` 定义类型化 `LightingPreset` 与 Dawn / Noon / Dusk / Night 配置。`LightingState.exposure` 在 fragment shader 中乘到 Normal-driven diffuse、Key 与 Ambient 合成后的环境结果，作为场景光照强度参数；它不是独立蓝色滤镜。Base 和 Lighting-off 仍在 shader 早退，保持与原图一致。Debug preset 快捷键应用完整静态参数，之后保留手动调节。本阶段没有时钟、插值、自动流动、Emission、Mask 或后处理。验证见 [`validation/TIME_OF_DAY_KEYFRAMES_R2A.md`](validation/TIME_OF_DAY_KEYFRAMES_R2A.md)。
