...
## R5 material response

`scripts/paint_material_mask.py` generates one 1672x941 RGBA texture: R is face/ear skin, G is crown hair, B is two irises, A unused. The hand-registered polygons and color guards prevent effects on bangs, hat, rose, shirt, and background. `HeroCanvas` checks dimensions and loads the texture; `BaseRenderer` binds it at texture unit 10. In Lit view, the shader samples the mask at the shared Breath/Hair-deformed `sampleUv` after Albedo and Normal. The face receives a mild skin color response, masked four-neighbor Normal smoothing and a soft painted light band. Crown hair receives a curved, directional, time-responsive sheen; irises receive a small key/ambient glint only while Blink is open. The Debug `Lighting Detail on/off` switch is the reversible A/B control; OFF returns the original Lit math. Base and Normal views, Sky RGB/timeline, frozen lighting inputs, and animation code are unchanged. See `docs/validation/R5_LIGHTING_DETAIL.md`.

## R6 Scene and Post boundary

In Lit with Post enabled, `hero.frag.glsl` stops before display conversion and writes linear scene radiance in portable RGBM RGBA8 to a full-frame Scene FBO. A second R8 attachment carries bloom eligibility: open Sky is excluded, and the brightest white shirt is attenuated. The frozen Sky plates are display-referred; inverse ACES plus inverse scene exposure registers them into linear Scene before the single final Post ACES. `PostPipeline` extracts with a soft knee at half resolution, then downsamples and blurs at 2/4/8/16 scales before weighted HDR addition. Final Post applies scene exposure, a small continuous post exposure offset, ACES once, sRGB conversion, restrained display saturation/contrast/tint, and the existing Night edge-tone correction in display space. Base and Normal bypass Post. Post OFF selects the exact old Lit output path. See `docs/validation/R6_POST_PROCESSING.md`.
