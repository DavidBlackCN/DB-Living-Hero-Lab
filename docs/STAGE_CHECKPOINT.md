...
| R2A.2 Lighting Visual Calibration | **工程实现完成，等待人工视觉验收**；场景光照与显示曝光 stops 已分离，增加 ACES-fit tone mapping、晨昏光能和可调 painted-band 参数。连续时间轨迹保持不变。 |
| Half Blink | **Optional / Later**；不是当前 Blink 验收前提。 |
| Masks / Regions | **TBD**；只在 Runtime Lighting 或动效实现确有需要时补充，并沿用 Artwork Space。 |

**Runtime Lighting Foundation v1 工程实现已通过。R2A 工程实现通过，但静态时段视觉校准未通过。R2A.1 工程实现完成但视觉校准未通过；R2A.2 已完成本轮工程校准，等待人工视觉验收，验收前不得进入 R2B。** Debug 24H Slider 当前仅供人工拉条校准，不跟随系统时间、不播放，也没有自动 timeline 驱动。当前不进入系统时间同步、自动播放、Bloom、Breathing、Hair Motion、Leaves v2 或其他动效。最终迁移至 VuePress 2 / `vuepress-theme-plume`。保持 Vue 页面层轻薄，`src/engine/` 可迁移。
...
R2A.2 曝光 / tone mapping 与人工校准结果见 [`validation/R2A2_LIGHTING_VISUAL_CALIBRATION.md`](validation/R2A2_LIGHTING_VISUAL_CALIBRATION.md)。
