...
`prefers-reduced-motion` is read at initialization and observed for changes. Reduced motion or disabling the animation master skips interpolation. The master currently controls time transitions only: blink, breathing, hair motion, steam and bloom are not implemented in this Lighting MVP.
