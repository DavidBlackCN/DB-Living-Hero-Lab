...
| `light-shaping.svg` | R window access, G exposed desktop coverage for inspection, B book/cup contact occlusion clipped to that desktop; 1200×675 |
...
Light-shaping channels are composed independently with screen blending. B has a 2.2-reference-pixel contact blur, clipped to exposed desktop, and contributes at most 12% darkening. G is no longer translated or sampled as a caster: that approximation made false hand/page shadows without receiver depth. G exports the desktop mask used when generating B for inspection. Its source texture adds 2.32 MiB RGB8. `lightShapingUrl` must retain the artwork aspect ratio and exact spatial registration. See the [desk correction](../../../docs/logs/desk-light-review.md).
