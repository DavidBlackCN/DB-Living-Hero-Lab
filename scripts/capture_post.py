"""R6 four-anchor A/B and optional full-composition motion captures."""
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        for phase in ("Dawn", "Noon", "Dusk", "Night"):
            ...
            post.check()
            bloom.uncheck()
            page.screenshot(path=str(args.output / f"{phase.lower()}-scene-post.png"))
            bloom.check()
            page.screenshot(path=str(args.output / f"{phase.lower()}-final.png"))
            preview.select_option("bright")
            page.screenshot(path=str(args.output / f"{phase.lower()}-bright.png"))
            preview.select_option("bloom")
            page.screenshot(path=str(args.output / f"{phase.lower()}-bloom.png"))
        ...
        if args.full:
            ...
            for phase in ("Dawn", "Noon", "Dusk", "Night"):
                ...
                if phase in ("Noon", "Dusk", "Night"):
                    ...
                    for _ in range(24):
                        ...
                        frames.append(frame.crop((760, 35, 1300, 660)).resize((432, 500)).quantize(colors=96))
                        match = re.search(r"Base frame: ([\d.]+) ms", page.locator(".debug-panel footer").inner_text())
                        if match:
            ...
            (args.output / "frame-times.json").write_text(json.dumps(times), encoding="utf-8")
        browser.close()
