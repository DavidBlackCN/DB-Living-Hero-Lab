export interface AssetOptions { baseUrl?: string; normalUrl?: string; maskUrl?: string; sceneMaskUrl?: string; lightShapingUrl?: string }
...
export async function loadAssets(options: AssetOptions) {
  const [base, normal, mask, sceneMask, lightShaping] = await Promise.all([
  ...
  return { base, normal, mask, sceneMask, lightShaping };
}
