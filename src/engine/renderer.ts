...
export type DebugView = 'final' | 'base' | 'normal' | 'masks' | 'lighting' | 'scene' | 'overlay' | 'bright' | 'bloom' | 'neutral';
export interface Settings { exposure: number; ambient: number; sun: number; lamp: number; normal: number; face: number; hair: number; cloth: number; night: number; refinement: number; stylized: number; softness: number; bloom: number; bloomThreshold: number; bloomRadius: number }
...
export async function createLivingHero(canvas: HTMLCanvasElement, options: HeroOptions = {}) {
  ...
  function render(now = performance.now()) {
    ...
    gl.uniform1i(loc('uView'),['final','base','normal','masks','lighting','scene','overlay','bright','bloom','neutral'].indexOf(view));
