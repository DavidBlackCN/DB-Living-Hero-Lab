...
const views: DebugView[] = ['final', 'scene', 'overlay', 'neutral', 'projected'];
...
test('acceptance captures: four times, five views, registered window closeups', async ({ page }) => {
  ...
  for (const view of ['base', 'final', 'scene', 'overlay'] as const) {
...
test('projected light moves spatially, reaches objects, and shuts off at night', async ({ page }) => {
  ...
  expect(morning.centroid[0]-noon.centroid[0]).toBeGreaterThan(40);
  expect(noon.centroid[0]-dusk.centroid[0]).toBeGreaterThan(30);
